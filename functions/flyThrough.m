function flyThrough(mat, dim, fps, loop,rectPos,xvec,yvec,xaxis,yaxis)
...
for loop_index = 1:loop
    ...
    for slice_index = 1:num_slices
        ...
        imagesc(xvec,yvec,slice, [-plot_scale, plot_scale]);
        rectangle('Position',rectPos);
        axis image;
        xlabel(xaxis);ylabel(yaxis);axis xy;
        title(['step =' num2str(slice_index) ' /' num2str(num_slices)])
        colormap(cmap);
        clr=colorbar;clr.Label.String='Pa'; 
        drawnow;
